...
void vTask1(void *pvParameters)
{
    ...
    // give some time to other tasks to initialise
    ...
    for(;;)
    {
        ...
        // busy wait for 10 ms
        busyDelay(10);
        // log job completion in case the semaphore is already claimed by another task
        logEvent(taskID, JOB_COMPLETION, (uint32_t)(xTaskGetTickCount() * portTICK_PERIOD_MS));
        // lock the resource
        if (NPPlock(sem1) == pdPASS)
        {
            // log job start after acquiring the semaphore to record preemptions caused by a release of the needed semaphore by a lower priority task
            logEvent(taskID, JOB_START, (uint32_t)(xTaskGetTickCount() * portTICK_PERIOD_MS));
            printf("Task 1 accessed shared resource\n");
            // keeps the semaphore for 30ms
            busyDelay(30);
            // log job completion to record preemptions by a higher priority task that is waiting on the semaphore
            logEvent(taskID, JOB_COMPLETION, (uint32_t)(xTaskGetTickCount() * portTICK_PERIOD_MS));
            // unlock the resource
            NPPunlock(sem1);
            // log job start to record the end of preemptions caused by releasing the semaphore
            logEvent(taskID, JOB_START, (uint32_t)(xTaskGetTickCount() * portTICK_PERIOD_MS));
        }
        else
        {
