...
void vCounter() {
    ...
    TickType_t previousWakeTime = xTaskGetTickCount();
    TickType_t period = pdMS_TO_TICKS(3500);

    while(true) {

        vTaskDelayUntil(&previousWakeTime, period);

        logEvent(taskID, JOB_START,
                 (uint32_t)(xTaskGetTickCount() * portTICK_PERIOD_MS));

        printf("Number of vCounter task execution: %d \r\n", count);
        count++;

        logEvent(taskID, JOB_COMPLETION,
                 (uint32_t)(xTaskGetTickCount() * portTICK_PERIOD_MS));
...
int main() {
    stdio_init_all();
    xTaskCreate(vLedBlink, "LedBlink", configMINIMAL_STACK_SIZE, NULL, 2, NULL);
    xTaskCreate(vCounter, "TaskExecusionCount",
